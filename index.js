        ...
        try {
            ...
            if (confidence >= 80 && (!signalBuffer.has(configKey) || (now - signalBuffer.get(configKey).timestamp > SIGNAL_COOLDOWN))) {
                ...
                if (!safeChatId || !safeSymbol || !safePair || !safeTimeframe || !safeSignal || safeConfidence === null || safeTimestamp === null || safeEntryPrice === null) {
                    console.error(`❌ Dữ liệu không hợp lệ khi lưu tín hiệu giả lập ${safeSymbol}/${safePair}: `, {
                ...
                db.run(
                    ...
                    (err) => {
                        if (err) {
                            console.error(`❌ Lỗi lưu tín hiệu giả lập ${safeSymbol}/${safePair} vào database: ${err.message}`);
                        } else {
                            console.log(`✅ Gửi tín hiệu giả lập ${safeSymbol}/${safePair} cho chat ${safeChatId} (Độ tin: ${safeConfidence}%)`);
                        }
                    }
                );
            }
...
bot.onText(/\/status/, (msg) => {
    try {
        ...
        const statusMessage = `
📊 *Trạng thái Bot*
- Số lần huấn luyện: ${trainingCounter}/${trainingLimit}
- Độ chính xác trung bình: ${(avgAcc * 100).toFixed(2)}%
- Độ chính xác cao nhất: ${(maxAcc * 100).toFixed(2)}%
- Độ chính xác thấp nhất: ${(minAcc * 100).toFixed(2)}%
